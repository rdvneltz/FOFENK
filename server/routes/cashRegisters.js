...
router.post('/transfer', async (req, res) => {
  try {
    ...
    const outgoingExpense = new Expense({
      ...
      status: 'paid', // Transfers are immediate, not pending
...
router.post('/transactions/:id/delete', async (req, res) => {
  try {
    ...
    if (transactionType === 'Payment') {
      ...
      // 3. Check if this is a transfer record - DO NOT reverse balance for transfers
      // Transfer balance was already changed during the transfer operation
      const isTransferRecord = transaction.isTransfer === true ||
                               transaction.category === 'Virman (Çıkış)' ||
                               transaction.category === 'Virman (Giriş)';

      if (isTransferRecord) {
        // For transfers, just delete the record but don't change balance
        deletionDetails.push(`Virman kaydı silindi (bakiye değiştirilmedi - virman zaten gerçekleşti)`);
      } else {
        // Reverse cash register balance (add back expense amount) - only for non-transfer expenses
        cashRegister.balance += transaction.amount;
        await cashRegister.save();
...
// Migration endpoint for old transfers (creates Expense records WITHOUT modifying balances)
router.post('/migrate-old-transfers', async (req, res) => {
  try {
    const { transfers } = req.body;
    // transfers: [{ date, fromCashRegisterName, toCashRegisterName, amount, description }]

    console.log('Migration request received:', JSON.stringify(transfers, null, 2));

    if (!transfers || !Array.isArray(transfers)) {
      return res.status(400).json({ message: 'Transfers array is required' });
    }

    // Get all cash registers first for better matching
    const allCashRegisters = await CashRegister.find({});
    console.log('Available cash registers:', allCashRegisters.map(c => c.name));
    ...
    for (const transfer of transfers) {
      try {
        const { date, fromCashRegisterName, toCashRegisterName, amount, description } = transfer;

        // Find cash registers by name (case-insensitive, trim whitespace)
        const fromCashName = fromCashRegisterName.trim().toLowerCase();
        const toCashName = toCashRegisterName.trim().toLowerCase();

        const fromCash = allCashRegisters.find(c => c.name.trim().toLowerCase() === fromCashName);
        const toCash = allCashRegisters.find(c => c.name.trim().toLowerCase() === toCashName);

        if (!fromCash) {
          errors.push(`Kaynak kasa bulunamadı: "${fromCashRegisterName}" (mevcut kasalar: ${allCashRegisters.map(c => c.name).join(', ')})`);
          continue;
        }
        if (!toCash) {
          errors.push(`Hedef kasa bulunamadı: "${toCashRegisterName}" (mevcut kasalar: ${allCashRegisters.map(c => c.name).join(', ')})`);
          continue;
        }

        const transferDate = new Date(date);

        // Create outgoing expense record (does NOT modify balance)
        const outgoingExpense = new Expense({
          category: 'Virman (Çıkış)',
          amount: amount,
          description: description || `Virman: ${fromCash.name} → ${toCash.name}`,
        ...
        // Create incoming expense record (does NOT modify balance)
        const incomingExpense = new Expense({
          category: 'Virman (Giriş)',
          amount: amount,
          description: description || `Virman: ${fromCash.name} → ${toCash.name}`,
        ...
        results.push({
          success: true,
          transfer: `${fromCash.name} → ${toCash.name}: ₺${amount}`,
          date: transferDate.toISOString()
        });
      } catch (err) {
        errors.push(`Hata: ${transfer.fromCashRegisterName} → ${transfer.toCashRegisterName}: ${err.message}`);
      }
    }

    console.log('Migration completed. Results:', results.length, 'Errors:', errors);

    res.json({
      message: `${results.length} virman kaydı oluşturuldu`,
      results,
