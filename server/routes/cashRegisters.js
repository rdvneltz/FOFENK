  ...
  try {
    ...
    for (const transfer of transfers) {
      try {
        ...
        // Find cash registers by name
        const fromCash = await CashRegister.findOne({ name: fromCashRegisterName });
        const toCash = await CashRegister.findOne({ name: toCashRegisterName });

        if (!fromCash) {
          errors.push(`Kaynak kasa bulunamadı: ${fromCashRegisterName}`);
          continue;
        }
        if (!toCash) {
          errors.push(`Hedef kasa bulunamadı: ${toCashRegisterName}`);
          continue;
        }
