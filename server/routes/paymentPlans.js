...
// Delete a single installment from a payment plan
router.post('/:id/delete-installment', async (req, res) => {
  try {
    const { installmentNumber, deletedBy } = req.body;
    ...
    // If only one installment remains, don't allow deletion — use plan delete instead
    if (paymentPlan.installments.length <= 1) {
      return res.status(400).json({
        message: 'Son taksit silinemez. Bunun yerine ödeme planını silebilirsiniz.'
      });
    }

    const instAmount = installment.amount || 0;
    const instPaidAmount = installment.paidAmount || 0;
    const wasPaid = installment.isPaid || instPaidAmount > 0;

    // If installment had payments, reverse them
    if (wasPaid && instPaidAmount > 0) {
      // Find related Payment records for this installment
      ...
      for (const payment of relatedPayments) {
        ...
        payment.notes = (payment.notes || '') + ' (Taksit silindi)';
        await payment.save();
      }

      // Delete auto-generated expenses (commission, VAT) for these payments
      for (const payment of relatedPayments) {
        ...
        for (const expense of autoExpenses) {
          // Revert cash register balance for the expense
      ...
      // Revert student balance for the paid amount (student paid this, so add it back as credit)
      await Student.findByIdAndUpdate(paymentPlan.student._id || paymentPlan.student, {
        $inc: { balance: instPaidAmount }
      });
    }

    // Remove installment from array
    paymentPlan.installments = paymentPlan.installments.filter(
      inst => inst.installmentNumber !== installmentNumber
    );

    // Renumber remaining installments sequentially
    ...
    const oldDiscountedAmount = paymentPlan.discountedAmount;
    paymentPlan.discountedAmount = paymentPlan.installments.reduce(
      (sum, inst) => sum + (inst.amount || 0), 0
    );
    paymentPlan.paidAmount = paymentPlan.installments.reduce(
      (sum, inst) => sum + (inst.paidAmount || 0), 0
    );
    paymentPlan.remainingAmount = paymentPlan.discountedAmount - paymentPlan.paidAmount;
    ...
    // Adjust student balance for the debt difference (removed installment = less debt)
    const debtDifference = paymentPlan.discountedAmount - oldDiscountedAmount; // negative
    if (debtDifference !== 0 && paymentPlan.student) {
      await Student.findByIdAndUpdate(paymentPlan.student._id || paymentPlan.student, {
        $inc: { balance: debtDifference }
      });
    }
    ...
    await ActivityLog.create({
      ...
      entity: 'PaymentPlan',
      entityId: paymentPlan._id,
      description: `${deletedBy || 'System'} tarafından ${studentName} için ${installmentNumber}. taksit silindi (₺${instAmount.toLocaleString('tr-TR')}${wasPaid ? ' - ödenmiş taksit geri alındı' : ''})`,
      metadata: {
        studentName,
        deletedInstallmentNumber: installmentNumber,
        deletedAmount: instAmount,
        wasPaid,
        paidAmountReversed: wasPaid ? instPaidAmount : 0
      },
      institution: paymentPlan.institution,
      season: paymentPlan.season
    });
    ...
    res.json({
      message: `${installmentNumber}. taksit silindi${wasPaid ? ' ve ödeme geri alındı' : ''}`,
      paymentPlan: populatedPaymentPlan
    });
  } catch (error) {
